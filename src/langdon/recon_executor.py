...
def _download_android_binaries(*, manager: LangdonManager) -> None:
    ...
    for android_app_id in manager.session.scalars(android_apps_query):
        with shell_command_execution_context(
            CommandData(command="apkeep", args=f"{android_app_id} {android_bin_dir}"),
            manager=manager,
        ) as output:
            ...
...
def _discover_domains_from_known_ones_passively(*, manager: LangdonManager) -> None:
    ...
    with tempfile.NamedTemporaryFile("w+") as temp_file:
        ...
        with shell_command_execution_context(
            CommandData(command="amass", args=f"enum -df {temp_file.name}"),
            manager=manager,
        ) as output:
        ...
        with shell_command_execution_context(
            CommandData(command="subfinder", args=f"-silent -dL {temp_file.name}"),
            manager=manager,
        ) as output:
...
def _discover_domains_actively(*, manager: LangdonManager) -> None:
    known_domains_query = sql.select(Domain.name)
    known_domain_names = manager.session.scalars(known_domains_query).all()
    ...
    with tempfile.NamedTemporaryFile("w+") as temp_file:
        ...
        with shell_command_execution_context(
            ...
            generated_domains = output.splitlines()
...
def _discover_content_actively(*, manager: LangdonManager) -> None:
    known_urls_query = wd = sql.select(
